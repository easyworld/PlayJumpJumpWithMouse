...
public class BackgroundImage4Panel extends javax.swing.JFrame {
	...
	public static void main(String[] args) {
		...
		opt = new Option("a", "adb-path", true,
				"adb path in system(including adb file name), eg: C:\\Users\\RoyZ\\Desktop\\platform-tools\\adb.exe");
		opt.setRequired(true);
		options.addOption(opt);

		opt = new Option("o", "screenshot-path", true,
				"screenshot path(including filename), eg: C:\\Users\\RoyZ\\Desktop\\untitled\\s.png");
		opt.setRequired(false);
		options.addOption(opt);
		...
		opt = new Option("t", "interval", true, "screenshot interval(ms), eg: 2500");
		opt.setRequired(false);
		options.addOption(opt);
		...
		JPanel jPanel = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				try {
					...
					System.out.println("IOException: Please check " + screenshotPath);
					e.printStackTrace();
				}
			}
		};
